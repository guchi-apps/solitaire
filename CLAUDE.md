...
| 状態 | 意味 | 入る内容 |
|---|---|---|
| 未設定 | ローカルの `npm version` / `npm run build` | `（更新内容を記入してください）`（手で置き換える） |
| 設定済み・空文字 | ワークフロー経由・利用者向けの変化なし | `内部構造を変更しました。` の1行 |
...
**既定文は `内部構造を変更しました。` から変えないこと。内訳は書かない。** ユーザー向けの変更が
1つも無いリリースでは更新履歴のエントリ自体を作らないのが原則だが、このリポジトリは
`tests/changelog.test.js` が `CHANGELOG[0].version === package.json の version` を強制していて
先頭エントリを省略できないため、この1行だけを置く決まりになっている。「安定性を改善」のような
利用者が確認できない抽象的な改善を書き足さない。

**この既定文だけは、バンプPRがdevelopへマージされるまで書き換えてよい**（`js/changelog.js` の
「過去バージョンのエントリは変更しない」の対象外）。上げ幅の自動判定が失敗したときも
`RELEASE_CHANGELOG` は空で渡り、判定成功で空だった場合と区別できない。バンプPRはCI通過後に
自動マージされ、それ以降エントリは変更禁止になるため、**バンプPRの本文にある判断根拠（reason）を
読み、利用者向けの変更が含まれていたらマージ前に書き直す。**
